...
u32 __ctru_linear_heap_size = 0x400000;
...
void __system_allocateHeaps(void) {
	...
	if(envIsHomebrew()) {
		...
		__ctru_heap_size = envGetHeapSize() + (envGetLinearHeapSize() - __ctru_linear_heap_size);
	} else {
		// Distribute available memory into halves, aligning to page size.
		u32 size = (osGetMemRegionFree(MEMREGION_APPLICATION) / 4) & 0xFFFFF000;
		__ctru_heap_size = size * 3;
